...
final class GameService
{
    ...
    public function decide(int $userId, int $sessionId, string $left, string $right, string $winner, string $reason): void
    {
        ...
        $sessions=Database::table('battle_sessions');
        $stmt=Database::pdo()->prepare("SELECT * FROM {$sessions} WHERE id=:id AND user_id=:u AND status='active'");
        $stmt->execute(['id'=>$sessionId,'u'=>$userId]);$session=$stmt->fetch();
        if(!$session) throw new \RuntimeException('Dieses Duell ist nicht mehr aktiv.');
        ...
        try{
            ...
            $pdo->prepare("UPDATE {$sessions} SET status='completed',completed_at=:t WHERE id=:id AND user_id=:u")
                ->execute(['t'=>gmdate('Y-m-d H:i:s'),'id'=>$sessionId,'u'=>$userId]);
            $pdo->commit();
